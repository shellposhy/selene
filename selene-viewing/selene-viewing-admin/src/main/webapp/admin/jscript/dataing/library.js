	...
	if ($("#directoryTree").length > 0) {
		$.ajax({
			...
			success : function(data) {
				if (data.data != null) {
					menuTreeCom($("#directoryTree"), data.data, true, bindTreeNodeEvent,"/admin/library/");
				}
			}
		});
	}
...
function library_data_copy(dataTable) {
	...
	$("#data_copy")
			...
			.bind(
					'click',
					function() {
						var findChecks = $(this).parent().nextAll(
								".dataTables_wrapper");
						if (findChecks.length < 1) {
							findChecks = $(this).parent().parent().nextAll(
									".dataTables_wrapper");
						}
						if (findChecks.find("table input[type='checkbox']").length > 0) {
							...
							findChecks.find(
									"table tbody input[type='checkbox']").each(
									function() {
										...
										.ajax({
											...
											success : function(data) {
												...
												if (null != data
														&& data.length > 0) {
													for (var i = 0; i < data.length; i++) {
														optionStr += '<input type="radio" name="baseIds" value="'
																+ data[i].id
																+ '">'
																+ data[i].name
																+ '&nbsp;';
...
function bind_search() {
	$("#search_u_db_btn")
			.click(
					function() {
						if ($("#search_u_db").val()) {
							$("#categoryTree .curSelectedNode").removeClass(
									"curSelectedNode");
							var sdata = '[{"name":"sSearch","value":"'
									+ $("#search_u_db").val() + '"}]';
							$
									.ajax({
										type : "post",
											...
											if (data.length != 0) {
												...
												for (var i = 0; i < data.length; i++) {
													...
													editLink += "	<span class='dbtime'>更新时间：<br />"
															+ data[i].dataUpdateTimeStr
															+ "</span>";
													editLink += "	<div class='actions' >";
													editLink += "		<a class='btn btn-small db_edit' href='"
															+ thisPath
															+ "edit/"
															+ data[i].id
															+ "' target='_self'><i class='icon-pencil'></i>修改</a>";
													editLink += "		<a class='btn btn-small ml5 db_del' href='#' onclick='delete_lib("
															+ data[i].id
															+ ")' target='_self'><i class='icon-trash'></i>删除</a>";
												...
												$("#libraries")
														.html(
																"<h3 class='alert alert-info' >无匹配的搜索结果！</h3>");
											}
										}
									});
						} else {
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						if (dataSize == fieldsIds.length
								&& fieldsIds.length == 0) {
							for (var i = 0; i < dataSize; i++) {
								temCheckCon += "<label><input  type='checkbox'   onclick='changeChkVal()'   rel='Reason' name='chk'   value='"
										+ dataResult[i].id
										+ "'  >"
										+ dataResult[i].name + "</label> ";
							}
						} else if (dataSize == fieldsIds.length
								&& fieldsIds.length > 0) {
							for (var i = 0; i < dataSize; i++) {
								temCheckCon += "<label><input  type='checkbox'   onclick='changeChkVal()'  checked  rel='Reason' name='chk'   value='"
										+ dataResult[i].id
										+ "'  >"
										+ dataResult[i].name + "</label> ";
							}
