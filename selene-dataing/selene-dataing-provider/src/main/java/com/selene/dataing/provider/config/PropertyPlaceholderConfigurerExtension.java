...
public class PropertyPlaceholderConfigurerExtension extends PropertyPlaceholderConfigurer {
	...
	protected void processProperties(ConfigurableListableBeanFactory beanFactoryToProcess, Properties props)
		...
		// Database user name
		String userName = props.getProperty(DatabaseConstants.DB_USERNAME_KEY);
		if (!isNullOrEmpty(userName)) {
			props.setProperty(DatabaseConstants.DB_USERNAME_KEY, decrypt(userName, DatabaseConstants.SELENE_AUTH));
		}
		// Database password
		String userPass = props.getProperty(DatabaseConstants.DB_PASSWORD_KEY);
		if (!isNullOrEmpty(userPass)) {
			props.setProperty(DatabaseConstants.DB_PASSWORD_KEY, decrypt(userPass, DatabaseConstants.SELENE_AUTH));
		}
