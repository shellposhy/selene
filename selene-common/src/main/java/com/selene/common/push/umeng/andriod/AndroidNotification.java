...
import com.selene.common.push.umeng.Notification;
...
public abstract class AndroidNotification extends Notification {
	...
	public enum DisplayType {
		// After the message is sent to the user's device, Umeng SDK takes
		// over the processing and displays the notification content on the
		// notification bar.
		NOTIFICATION {
		...
		// Once the message reaches the user's device, the message content is
		// passed through to the application itself for parsing.
		MESSAGE {
			public String getValue() {
				return "message";
			}
	...
	public enum AfterOpenAction {
		go_app, // Open app
		go_url, // Redirect URL
		go_activity, // Turn on specific activities
		go_custom// User-defined content
	}
